import {
  ...
  Download, CheckCircle2, Navigation, Route as RouteIcon,
} from "lucide-react";
...
export default function TripPlanDetail({ plan, from, to, days, food, group, transport, withGuide, payment }) {
  ...
  return (
    <div className="rounded-2xl bg-card ring-1 ring-border p-5 space-y-5">
      ...
      {/* How to reach */}
      <Block icon={reach.icon} title="How to reach">
        <p className="text-sm font-semibold text-foreground">{reach.mode}</p>
      ...
      {hotel && (
        <Block icon={BedDouble} title="Where to stay">
          ...
          <p className="text-xs text-muted-foreground">{hotel.city} · {hotel.amenities} · ₹{hotel.price}/night</p>
          <a href={mapsLink(`${hotel.name} ${hotel.city}`)} target="_blank" rel="noreferrer"
            className="mt-2 inline-flex items-center gap-1.5 text-xs font-semibold text-primary hover:underline">
      ...
      <div className="pt-4 border-t border-border space-y-1.5 text-sm">
        <Row label="Hotel" v={plan.breakdown.hotelCost} />
        <Row label="Transport" v={plan.breakdown.tCost} />
        <Row label="Food" v={plan.breakdown.foodCost} />
        {plan.breakdown.guideCost > 0 && <Row label="Guide" v={plan.breakdown.guideCost} />}
